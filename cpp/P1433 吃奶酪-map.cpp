...
struct node2{
	...
	bool operator<(const node2 tt)const{
		if(id!=tt.id) return id<tt.id;
		for(int i=1;i<=n;i++){
			if(in[i]!=tt.in[i]) return in[i]<tt.in[i]; 
		}
	} 
	bool operator == (const node2 tt)const{
		if(id!=tt.id) return 0;
		for(int i=1;i<=n;i++){
			if(in[i]!=tt.in[i]) return 0; 
		}
		return 1;
	} 
}vis;
__gnu_pbds::gp_hash_table<node2,int> h;
int tot;
